/*
 *======================================================
 * Demo of procedural shading in OpenGL and GLSL
 * Author: Stefan Gustavson (stefan.gustavson@gmail.com)
 * 2011-10-21. This code is in the public domain.
 *======================================================
 */
...
/*
 * loadExtensions - Load (or check for) the required OpenGL extensions.
 */
void loadExtensions() {
 ...
 */
 void setUniformVariables(GLhandleARB programObj,
                          int texunit, float texw, float texh) {

  GLint location_time = -1;
  GLint location_teximage = -1;
  ...
  // current time in seconds, one texture and two integers
  // telling the texture size
  location_time = glGetUniformLocationARB( programObj, "time" );
  if(location_time != -1)
    glUniform1fARB( location_time, (float)glfwGetTime() );

  location_teximage = glGetUniformLocationARB( programObj, "teximage" );
  if(location_teximage != -1)
    glUniform1iARB( location_teximage, texunit );
...
int main(int argc, char *argv[])
{
  ...
  loadTexture(PATH "texture.tga", textureID, &texw, &texh);
  
  // Create, load and compile the shader program
  programObj = createShader(PATH "vertex.glsl", PATH "fragment8.glsl");
  
  // Enable vertical sync (on cards that support
  ...
  while( running )
    {
      ...
      if(glfwGetKey('5')) {
        loadTexture(PATH "texture.tga", textureID, &texw, &texh);
        programObj = createShader(PATH "vertex.glsl", PATH "fragment5.glsl");
      }
      if(glfwGetKey('6')) {
        loadTexture(PATH "texture.tga", textureID, &texw, &texh);
        programObj = createShader(PATH "vertex.glsl", PATH "fragment6.glsl");
      }
      if(glfwGetKey('7')) {
        loadTexture(PATH "texture.tga", textureID, &texw, &texh);
        programObj = createShader(PATH "vertex.glsl", PATH "fragment7.glsl");
      }
      if(glfwGetKey('8')) {
        loadTexture(PATH "texture.tga", textureID, &texw, &texh);
        programObj = createShader(PATH "vertex.glsl", PATH "fragment8.glsl");
