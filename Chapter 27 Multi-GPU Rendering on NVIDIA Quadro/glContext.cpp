...
glContext::createRenderingContext(HDC theDC)
{
	glContext::renderingContext *rc;

	rc = new renderingContext();
	rc->_hDC = theDC? theDC : GetDC(window::getInstance().hWnd());
	if (_hDC!= rc->_hDC) { //set pixel format again for the new DC
		_pixelFormat = ChoosePixelFormat(_hDC, &pfd);
		if(_pixelFormat == 0)
		{
			window::getInstance().messageBoxWithLastError("ChoosePixelFormat");
		...
		if(SetPixelFormat(rc->_hDC, _pixelFormat, &pfd) == FALSE)
		{
			window::getInstance().messageBoxWithLastError("SetPixelFormat");
			return false;
		}
