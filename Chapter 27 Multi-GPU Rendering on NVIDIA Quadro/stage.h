...
protected:
	...
	HDC _affinityDC; //some stages may have their own affinityDC if using multiGPU
	...
	/// Called before the forall-streams to do any drawing init etc
	virtual bool startFrame() { return true ;};
    /// The stage work function that is done in a loop repeatedly
    virtual bool execute(unsigned int stream) = 0;
	/// Called after the forall-streams to do any drawing init etc
...
public:
	...
	void setAffinityDC(HDC affDC) { _affinityDC = affDC;}
	const glContext::renderingContext *getRC() {
		return _rc;
	}
