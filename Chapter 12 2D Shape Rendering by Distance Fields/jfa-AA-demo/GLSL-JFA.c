/*
 *======================================================
 * Demo of jump flooding algoritm for AA-EDT using GLSL
 * Author: Stefan Gustavson (stefan.gustavson@gmail.com)
 * 2010-08-31. This code is in the public domain.
 *======================================================
 */
...
#include <GL/glext.h> // Caution: Might be outdated on many systems
...
void createBufferTexture(GLuint texID, int texw, int texh) {
  ...
  glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16, texw, texh, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0 );
...
int main(int argc, char *argv[])
{
  ...
  // Load OpenGL extensions (requires an OpenGL context, i.e an open window)
  loadExtensions();
  ...
  loadShapeTexture(PATH "shape1.tga", textureID[0], &texw, &texh);
  ...
  programObj[0] = createShader(PATH "vertex.glsl", PATH "fragment_seed.glsl");
  programObj[1] = createShader(PATH "vertex.glsl", PATH "fragment_flood.glsl");
  programObj[2] = createShader(PATH "vertex.glsl", PATH "fragment_display.glsl");
  ...
  while( running )
    {
...
#else
           // Flooding, step size 1 and lots of iterations (slow, for reference only)
           stepsize = (texw > texh ? texw: texh); // Actually "number of steps" here
           while (stepsize > 0) {
               setUniformVariables( programObj[1], 0, (float)texw, (float)texh,
                                    (float)65536, (float)1 ); // Step size always 1
               glBindTexture( GL_TEXTURE_2D, textureID[lastRendered] );
               lastRendered = (lastRendered == 1 ? 2 : 1); // Swap 1 <-> 2
               // Swap which texture is attached to the FBO
               glFramebufferTexture2D( GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
      ...
      if(glfwGetKey('2')) {
        loadShapeTexture(PATH "shape2.tga", textureID[0], &texw, &texh);
        createBufferTexture(textureID[1], texw, texh);
        createBufferTexture(textureID[2], texw, texh);
      }
      if(glfwGetKey('3')) {
        loadShapeTexture(PATH "shape3.tga", textureID[0], &texw, &texh);
        createBufferTexture(textureID[1], texw, texh);
        createBufferTexture(textureID[2], texw, texh);
      }
      if(glfwGetKey('4')) {
        loadShapeTexture(PATH "shape4.tga", textureID[0], &texw, &texh);
        createBufferTexture(textureID[1], texw, texh);
        createBufferTexture(textureID[2], texw, texh);
      }
