...
Timer::update()
{
	...
	if((double)(currentCPUTime.QuadPart - _startCPUTime.QuadPart) / (double)_frequency.QuadPart > _options._runTime)
	{
		...
		//TODO which one to choose - upload or readback?
		const float dataSize = _options._inTexWidth * _options._inTexHeight * 4.f;
...
Timer::information(char *string, size_t stringlen)
{
	...
	const float dataSize = (_options._inTexWidth * _options._inTexHeight * 4.f)/(1024.f * 1024.f);

	sprintf_s(string, stringlen, "%u iterations, CPU %4.2f buffers/s, %8.2f Mbyte/s",
		_options._maxIterations
